...
outputfilePath = 'annual mean total precipitation classification.png'
...
# 设置不同降水水平的颜色和数值范围
levels = [0, 200, 400, 800, 12000]
colors = ['#FAC38D', '#FFF6C1', '#B0C9D4', '#95C678']
# 为不同水平创建颜色映射
...
contourf_ChinaMap = ax_ChinaMap.contourf(lon, lat, data, levels=levels, cmap=cmap, norm=norm)

# 添加九段线子图
ax_chinaNineLine = figure.add_axes([0.79, 0.2338, 0.15, 0.3], projection=ccrs.PlateCarree())
...
ax_chinaNineLine.contourf(lon, lat, data, levels=levels, cmap=cmap, norm=norm)

# 设置颜色条和标签
cbar = plt.colorbar(contourf_ChinaMap, ax=ax_ChinaMap, ticks=[100, 300, 600, 6400], orientation="horizontal",
                    shrink=0.85, aspect=34, pad=.08)
cbar.ax.set_xticklabels(['干旱区', '半干旱区', '半湿润区', '湿润区'],
